...
public class Cryptos {
    ...
    private static final Logger logger = LoggerFactory.getLogger(Cryptos.class.getName());
     ...
     * @Author gg.rao
     * @Description 云南信托加签方法
     * @Date 2018/11/5 11:26
     * @Param [body, privatePath, privateKey]
     * body=内容摘要、私钥证书路径 、privateKey 私钥
     **/
    public static String md5withRSA(String body, String privatePath, String privateKey) {
        byte[] signedInfo = null;
        try (FileInputStream fis = new FileInputStream(privatePath)) {
            KeyStore ks = KeyStore.getInstance("PKCS12");
            char[] nPassword;
            if (StringUtils.isEmpty(privateKey)) {
                nPassword = null;
            } else {
                nPassword = privateKey.toCharArray();
            }
            ...
            logger.error("md5withRSA failed.", e);
        }

        return Encodes.encodeBase64(signedInfo);
    }
}
